...
export default function LockedFeatureNotice({ tier }: { tier: 'business' | 'enterprise' }) {
  const { t } = useI18n()
  const badge = tier === 'enterprise' ? 'bg-blue-600' : 'bg-green-600'
  return (
    <div className="max-w-xl rounded-lg border border-border bg-surface p-6">
      <div className="flex items-center gap-3">
        ...
        <span className={`rounded-full px-2 py-0.5 text-xs font-semibold uppercase tracking-wide text-white ${badge}`}>
